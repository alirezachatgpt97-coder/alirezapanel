...
**alirezapanel** یک Integration سبک، تک‌فایلی و حرفه‌ای برای ترکیب
**vpn-ui** و **AdGuard Home** پشت یک Gateway واحد است؛ بدون جایگزین‌کردن
قابلیت‌های upstream و بدون نصب Docker، Node.js، npm یا build toolchain
روی سرور مقصد.

![Version](https://img.shields.io/badge/alirezapanel-1.4.0-ff7a18?style=for-the-badge)
![VPN
UI](https://img.shields.io/badge/vpn--ui-v1.9.4-2f80ed?style=for-the-badge)
![AdGuard
Home](https://img.shields.io/badge/AdGuard%20Home-v0.107.79-67b279?style=for-the-badge)
...
> ⚡ طراحی‌شده برای اجرای سبک روی VPSهای کم‌منبع؛ **1 vCPU / 1 GiB RAM**
> هدف عملی پروژه است، با این نکته که ظرفیت واقعی به پروتکل‌ها، تعداد
> کاربران، فیلترها و حجم ترافیک بستگی دارد.
:::

------------------------------------------------------------------------

## 🧡 درباره پروژه و توسعه

**alirezapanel** با هدف ساخت یک تجربه مدیریتی یکپارچه، سبک و حرفه‌ای
توسعه داده شده است. بخش Integration پروژه، Gateway، منطق اتصال اجزا،
مدیریت Nodeها، ابزارهای مدیریتی، پوسته و Branding، قابلیت‌های تکمیلی
کاربران، Subscription Studio و Installer تک‌فایلی در قالب کد پروژه
alirezapanel توسعه و نگهداری می‌شوند.

هدف پروژه صرفاً کنار هم قراردادن چند سرویس نیست؛ alirezapanel یک لایه
مدیریتی و عملیاتی مستقل ایجاد می‌کند تا نصب، دسترسی، امنیت، مدیریت
کاربران، DNS، Subscription، Nodeها، SSL، Repair و Health Check از یک
ساختار منسجم قابل کنترل باشند.

**توسعه و نگهداری پروژه: Alireza / alirezapanel**

> \[!NOTE\] alirezapanel برای برخی قابلیت‌های زیرساختی از نرم‌افزارهای
> متن‌باز و باینری‌های upstream استفاده می‌کند و مجوزها و Attribution آن‌ها
> را حفظ می‌کند. هویت، Integration و قابلیت‌های اختصاصی alirezapanel متعلق
> به خود پروژه هستند.
...
**alirezapanel** یک پنل جدید از صفر نیست؛ یک لایه‌ی Integration روی
ابزارهای upstream است.
...
-   امکانات تکمیلی مثل Node، Client Policy، گزارش مصرف و Subscription
    Studio اضافه شوند.
-   هیچ Framework سنگین frontend یا daemon اضافه‌ی بی‌دلیل وارد سیستم
    نشود.

یعنی پروژه تلاش نمی‌کند قابلیت‌های موجود را کم کند؛ بلکه قابلیت‌های جدید را
**در کنار امکانات اصلی** اضافه می‌کند.
...
فایل‌های اجرایی و رابط‌های اصلی vpn-ui و AdGuard Home حفظ می‌شوند. تنظیمات
پروتکل، API payloadها، Routing، Reality، TLS و ساختارهای اصلی VPN به‌صورت
خودسرانه بازنویسی نمی‌شوند.
...
Integration اصلی در همان `install.sh` قرار دارد و هنگام نصب فایل‌های لازم
استخراج می‌شوند.

### 3. کمترین Polling ممکن

قابلیت‌های اضافه تا حد ممکن **On-demand** هستند. گزارش مصرف کاربر یا
Subscription Studio فقط زمانی داده می‌خوانند که مدیر آن را باز کند.
...
Installer فقط برای نصب اولیه نیست؛ حالت‌های `--repair`، `--check`،
`--ssl`، `--enable-nodes` و `--self-test` نیز دارد.
...
ورود از طریق سیستم authentication پنل VPN انجام می‌شود. Gateway کوکی را
خودش decode نمی‌کند و برای دسترسی حساس دوباره مجوز را از endpointهای خود
vpn-ui بررسی می‌کند.

## 🎨 رابط هماهنگ

Theme اختصاصی پروژه با ظاهر نارنجی/زغالی روی بخش‌های Integration اعمال
می‌شود و تا حد ممکن Navigation و رفتار native پنل را حفظ می‌کند.
...
AdGuard Home به‌صورت کامل داخل پنل قرار می‌گیرد، نه یک داشبورد ناقص یا API
محدود.

## 🖥 Multi-node

Nodeهای سازگار را می‌توان به سرور اصلی متصل کرد و مدیریت Remote انجام
داد.

## 🛠 Repair و Health Check

ابزارهای CLI برای بررسی سرویس، تعمیر Integration و بازسازی بخش‌های
مدیریت‌شده وجود دارد.

------------------------------------------------------------------------

# 📊 لاگ و مصرف واقعی هر کاربر

یکی از قابلیت‌های جدید نسخه فعلی، نمایش **مصرف واقعی هر Client به‌صورت
جداگانه** است.

این قابلیت داده ساختگی تولید نمی‌کند و packet capture جدیدی هم اضافه
نمی‌کند؛ بلکه تا جای ممکن از شمارنده‌های native خود vpn-ui در جدول
`client_traffics` استفاده می‌کند.
...
برای این قابلیت daemon جدید یا worker دائمی ساخته نشده است. داده‌ها در
زمان بازشدن Dialog توسط مدیر خوانده می‌شوند.

### نکته مهم درباره عبارت «لاگ»

این بخش **لاگ مصرف و شمارنده‌های واقعی VPN** است، نه history کامل
وب‌سایت‌هایی که کاربر باز کرده است.
...
**Subscription Studio** نمای مدیریتی حرفه‌ای برای مشاهده وضعیت اشتراک
Client است.

این بخش فرمت native subscription را خراب یا جایگزین نمی‌کند؛ بلکه یک
لایه‌ی نمایشی حرفه‌ای برای مدیر فراهم می‌کند.
...
به‌جای یک لینک خام و غیرقابل‌فهم، مدیر می‌تواند وضعیت حساب را به‌شکل واضح و
حرفه‌ای مشاهده کند.

> فرمت‌های native subscription، JSON و Clash که خود vpn-ui ارائه می‌کند
> حفظ می‌شوند.
...
هر Client می‌تواند credential مجزا داشته باشد و بسته به تنظیمات از
قابلیت‌هایی مانند:
...
این URL با subscription VPN یکسان نیست و برای کلاینت‌های DoH استفاده
می‌شود.
...
نسخه فعلی vpn-ui استفاده‌شده در Installer برای amd64 Pin شده است؛ به همین
دلیل ARM توسط Installer فعلی پشتیبانی نمی‌شود.
...
512 MiB حالت حداقلی است و ظرفیت آن تضمین نشده است. پروتکل‌های بیشتر،
geofileهای بزرگ، filter listهای سنگین، تعداد زیاد Client و ترافیک بالا
RAM بیشتری مصرف می‌کنند.
...
این روش بهتر است چون قبل از اجرا می‌توانید فایل را مشاهده و self-test
کنید.
...
این حالت برای Certificate trusted روی IP عمومی در نسخه Installer فعلی در
نظر گرفته شده است.
...
اگر Certificate خارجی ارائه نشده باشد، Certificate self-signed ساخته
می‌شود و مرورگر هشدار trust خواهد داد.
...
پورت‌های VPN وابسته به Inboundها و پروتکل‌هایی هستند که خودتان فعال
می‌کنید.
...
Repair برای بازگردانی فایل‌های Integration و باینری‌های Pin‌شده استفاده
می‌شود و تنظیمات/کاربران موجود را تا حد طراحی پروژه حفظ می‌کند.
...
Installer در بخش‌های مدیریت‌شده Backup ایجاد می‌کند و از downgrade خاموش
databaseهای ناسازگار جلوگیری می‌کند.

برای Master/Node پیشنهاد می‌شود همه سرورها از نسخه یکسان Installer
استفاده کنند.
...
-   Repair در صورت تشخیص version mismatch خطرناک، بی‌صدا downgrade انجام
    نمی‌دهد.
...
پیکربندی اولیه روی مقادیر محافظه‌کارانه‌ای قرار می‌گیرد، از جمله cache کوچک
و retention کوتاه‌تر برای Query Log و Statistics.

با این حال **Go memory target یک hard memory cap نیست** و هیچ نرم‌افزاری
نمی‌تواند در تمام workloadها مصرف ثابت 1 GiB را تضمین کند.
...
1.  **1 GiB RAM تضمین ظرفیت نامحدود نیست.** تعداد زیاد Client، فیلترهای
    سنگین و coreهای مختلف RAM بیشتری می‌خواهند.
2.  ARM پشتیبانی نمی‌شود.
3.  Migration خودکار از هر نصب قبلی vpn-ui/x-ui/AdGuard تضمین نشده و
    Fresh Server توصیه می‌شود.
4.  DNS per-client نمی‌تواند هر نوع ترافیک رمزگذاری‌شده یا DNS خارج از
    Tunnel را شناسایی کند.
...
8.  رفتار protocolهای native همچنان تحت محدودیت‌ها و قابلیت‌های upstream
    است.
...
برای README حرفه‌ای پیشنهاد می‌شود تصاویر واقعی پنل را در مسیر زیر قرار
دهید:
...
> اگر هنوز تصاویر را آپلود نکرده‌اید، GitHub فقط placeholder شکسته نشان
> می‌دهد؛ بنابراین بهتر است بعد از تهیه Screenshotها این مسیرها را اضافه
> کنید.
...
Installer علاوه بر Version، SHA256 باینری‌های upstream را نیز بررسی
می‌کند.
...
# 📁 مسیرهای مهم
  ...
  `/etc/alirezapanel/access.json`     اطلاعات ورود اولیه / recovery
                                      record
...
باینری‌ها و پروژه‌های upstream لایسنس‌های مستقل خود را دارند و attribution
آن‌ها حفظ می‌شود.
...
**One panel. VPN. DNS. Nodes. Real client usage. Professional
...
برای اینکه صفحه GitHub پروژه ظاهر حرفه‌ای‌تری داشته باشد، تصاویر واقعی پنل
را در مسیر `docs/images/` قرار دهید. README برای تصاویر اصلی زیر آماده
شده است:
  ...
  📊 User Usage            `docs/images/user-usage.png`
  🔗 Subscription Studio   `docs/images/subscription-studio.png`
  🛡️ DNS                   `docs/images/dns-dashboard.png`
  🌐 Nodes                 `docs/images/nodes.png`

نمونه نمایش در GitHub:

``` html
<p align="center">
  <img src="docs/images/dashboard.png" alt="alirezapanel Dashboard" width="900">
</p>
```

> \[!IMPORTANT\] بهتر است از Screenshot واقعی نسخه منتشرشده استفاده شود
> تا README دقیقاً با رابطی که کاربر پس از نصب می‌بیند مطابقت داشته باشد.
